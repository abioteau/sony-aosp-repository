...
cd $AOSP_WORKSPACE
~/bin/repo init -u $AOSP_MIRROR_URL/platform/manifest.git --repo-url $REPO_MIRROR_URL/git-repo.git -b android-7.0.0_r30
cd .repo
...
~/bin/repo status
~/bin/repo forall -p -c git log --oneline android-7.0.0_r30..$GIT_BRANCH
